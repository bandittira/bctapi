...
@app.get("/getProductId/{product_code}")
async def getProductId(product_code: str) -> dict:
    with pyodbc.connect(conn_str) as conn:
        cursor = conn.cursor()
        cursor.execute(
            "SELECT TOP 1 ProductCode, ProductId FROM product WHERE ProductCode = ? ORDER BY ProductId DESC",
            (product_code),
        )
...
async def getAllProduct():
    with pyodbc.connect(conn_str) as conn:
        cursor = conn.cursor()
        cursor.execute(
            "SELECT ProductCode, ProductId FROM Product ORDER BY Id DESC",
        )
        columns = [column[0] for column in cursor.description]
        results = [dict(zip(columns, row)) for row in cursor.fetchall()]
...
@app.get("/getLatestProduct")
async def getProductIdLatest():
    with pyodbc.connect(conn_str) as conn:
        try:
            with pyodbc.connect(conn_str) as conn:
                cursor = conn.cursor()
...
async def get_product(id: str, productCode: str) -> dict:
    # id = 1000002 & productCode = NK
    try:
        conn = pyodbc.connect(conn_str)
        ...
        product_results = execute_select_query(
            conn, product_query, (int(id), productCode)
        )
        product_detail_results = execute_select_query(
            conn, product_detail_query, (int(id), productCode)
        )
        product_diamonds_results = execute_select_query(
            conn, product_diamonds_query, (int(id), productCode)
        )
        product_gems_results = execute_select_query(
            conn, product_gems_query, (int(id), productCode)
        )
        product_materials_results = execute_select_query(
            conn, product_materials_query, (int(id), productCode)
        )
